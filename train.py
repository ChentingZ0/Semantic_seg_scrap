...
if __name__ == "__main__":
    ...
    with open(os.path.join(Iron_path, "Iron_material/ImageSets/Segmentation/train.txt"),"r") as f:
        train_lines = f.readlines()
    with open(os.path.join(Iron_path, "Iron_material/ImageSets/Segmentation/val.txt"),"r") as f:
        val_lines = f.readlines()
    ...
    if True:
        ...
        #----------------------#
        if local_rank == 0:
            eval_callback   = EvalCallback(model, input_shape, num_classes, val_lines, Iron_path, log_dir, Cuda, \
